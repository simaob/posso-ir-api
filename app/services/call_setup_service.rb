class CallSetupService
  ...
  def call
    ...
    stores = Store.where(make_phone_calls: true).joins(:week_days).where(week_days: {day: week_day, active: true}).joins(:phones)
    ...
    stores.each do |store|
      ...
      times = [start_at]
      while times.last < end_at
        times << (times.last + store.phone_call_interval.minutes)
      end
